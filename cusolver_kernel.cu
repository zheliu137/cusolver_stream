...
int cusolver_c_stream_QR(const int m,  cuDoubleComplex *A_, const int nmat_ ) {
    ...
    // So we calculate lwork and allocate d_work before loop
    CUSOLVER_CHECK(
          cusolverDnZheevd_bufferSize(cusolverH[0], jobz, uplo, m, 
          &d_A[0], lda, &d_W[0], &lwork[0]));
    ...
    /* step 5: compute eigen-pair   */
    CUSOLVER_CHECK(cusolverDnZheevd(cusolverH[ist], jobz, uplo, m, 
                                    &d_A[i*lda*m], lda, &d_W[ist*m], 
                                    d_work[ist], lwork[0], &devInfo[ist] ));
